...
bool IsDualSymbol(string word) {//先判断是不是二元符号，比如++ --
...
bool IsSingleSymbol(const string &word) {

    if (IsDualSymbol(word))return true;
...
bool IsTypeIdentifier(const string& word) {
...
void SelectWord(const string& word, int num) {

    if (IsNumber(word)) return;         //判断这个string是不是数字

    if (IsSingleSymbol(word)) return;   //判断这个string是不是单符号
